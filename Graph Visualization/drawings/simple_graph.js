    ...
    if(that.selection) {
      object_selection = new THREE.ObjectSelection({
        ...
        clicked: function(obj) {
        	if(obj != null) {
                info_text.select = "Node Clicked " + obj.id;
                window.open(shikkhok_courses[2 * obj.id + 1]);
            } else {
//                delete info_text.select;
            }
        }
...
/*
    var node = new Node(0);
    node.data.title = "Shikkhok courses" + node.id;
    graph.addNode(node);
    drawNode(node);
    ...
    while(nodes.length != 0 && steps < that.nodes_count) {
      ...
      for(var i=1; i <= numEdges; i++) {
        ...
        if(graph.addNode(target_node)) {
          target_node.data.title = "Shikkhok.com" + target_node.id;
    ...
    */
	  ...
	  shikkhok_courses = new SHIKKHOK.courses();
	  
	   var node = new Node(0);
	    node.data.title = "Shikkhok Courses";
	    graph.addNode(node);
	    drawNode(node);
	    ...
	    var steps = 1;
	    var node = nodes[0];
	    nodes[0].data.title = "Shikkhok Courses.";
	    nodes[0].data.link = "http://www.shikkhok.com";
	    
	    while(nodes.length != 0 && steps < that.nodes_count) {
	      var numEdges = that.nodes_count;
	      for(var i=1; i <= that.nodes_count; i++) {
	        var target_node = new Node(i);
	        if(graph.addNode(target_node)) {
	          target_node.data.title = shikkhok_courses[2*i];
	          target_node.data.link = shikkhok_courses[2*i + 1];
	          drawNode(target_node);
	          nodes.push(target_node);
	          if(graph.addEdge(node, target_node)) {
	            drawEdge(node, target_node);
  ...
  function drawNode(node) {
    ...
    draw_object.scale.x = 3;
    draw_object.scale.y = 3;
    ...
    var theta = 360 / (that.nodes_count + 1);
    var area = Math.pi * r^2;
    draw_object.position.x = Math.floor(r * Math.cos(theta * node.id));
    draw_object.position.y = Math.floor(r * Math.sin(theta * node.id));
  ...
  function drawEdge(source, target) {
      material = new THREE.LineBasicMaterial( { color: 0x000000, opacity: 0.3, linewidth: 0.5 } );
    ...
    if(that.show_labels) {
      ...
      for(var i=0; i<length; i++) {
        ...
        if(node.data.label_object != undefined) {
          node.data.label_object.position.x = node.data.draw_object.position.x - 800;
